...
// Precomputed — data is static, no need to recalculate on every keystroke
const TOTAL_TERMS = CATEGORIES.reduce((s, c) => s + c.terms.length, 0);

// filter is already lowercased by the caller
function renderAll(filter) {
  ...
  document.getElementById('counter').textContent =
    filter ? `${totalVisible} из ${TOTAL_TERMS}` : `${TOTAL_TERMS} терминов`;
